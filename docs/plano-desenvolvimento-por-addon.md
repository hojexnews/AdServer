...
| `E11` | ONNX Runtime nativo no sidecar (substituir StubInferencer por OnnxInferencer) | ◻ pendente | `ml-optimization-engineer` | `ADR-0003 §B` · `README.md — Fase 2 pendências ('ONNX Runtime nativo no sidecar, hoje stub')` · `TX-4` | pendência de código real (a triagem G0 corrigiu o overclaim anterior): o wrapper `OnnxInferencer` **ainda não existe** — não há `services/ranker-sidecar/internal/onnx/onnx.go`, arquivo com build tag `//go:build onnx`, nem dependência `yalue/onnxruntime_go`; o sidecar sempre instancia `StubInferencer` (Score→0.0). Falta: criar o wrapper sob build tag + contraparte `!onnx` CGO-free (hermético, ADR-0002 §C) + wiring no main + parity contra o modelo real. A validação do binário linkado (CGO) exige `libonnxruntime.so.1` onde a lib nativa existir. |
...
- **ml E11 — ONNX Runtime nativo** (OnnxInferencer sob build tag, preservando build hermético `ADR-0002 §C`). **← próximo item de código de G0.** Dono: `ml-optimization-engineer`; gate `tech-lead-architect` + `parity`. Ref: `ADR-0003 §B`.
