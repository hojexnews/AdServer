...
- **platform-infra E8 — fechar item 4 do mandato:** **← próximo item de código de G0.** `platform/secrets/openbao/policy-aml-kyc.hcl` faltante; pipeline cosign real + SBOM (syft) + Trivy + ruleset Falco + Dockerfiles de produção por serviço; substituir `REPLACE_WITH_COSIGN_PUBLIC_KEY`. Dono: `platform-infra-engineer` (coadjuvado pelos donos de camada nos Dockerfiles). Ref: mandato item 4, `§2.7`, `CA-9`.
