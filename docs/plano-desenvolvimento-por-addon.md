...
- **frontend E9 + E10 — fail-closed do middleware** em produção sem `SESSION_SECRET` **e** alinhamento de stack §2.5 (Next 15.3.3→16, React 19.1→19.2, shadcn/ui sobre Base UI/React Aria, Zustand, Vercel AI SDK v5, axe+Playwright em CI). **← próximo item de código de G0.** Dono: `frontend-bff-engineer`; gate `security-reviewer` + `tech-lead-architect`. Ref: `§2.5`, `WCAG 2.2 AA`, `TX-3`.
