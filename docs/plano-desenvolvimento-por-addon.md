...
- **Contratos/Schema** (`schema-contracts-steward`): 5 pacotes proto verdes (common/telemetry/decision/money/payments); `buf lint/breaking/make verify` verdes contra `main`. Único ruído: `proto-gen-check` falso-positivo por bump de plugin remoto (E8).
- **Motor de decisão** (`decision-engine-engineer`): I0/I2/I5 + fiação do ranker (J0/J1/K1) atrás de flags default-off; 5 goldens CA-2..CA-6 completos. 2 itens de código genuínos: hot-reload GeoLite2 (E5) e HOT-1/HOT-3 (E6, **pré-condição bloqueante** antes de ligar flags).
...
- **ML** (`ml-optimization-engineer`): J0–J6 + K1/K2; pendências de código: HOT-1/HOT-3 (E10, espelha decision-engine) e ONNX Runtime nativo (E11, hoje StubInferencer).
...
- **Front/BFF** (`frontend-bff-engineer`): I4/J5/K7; 2 dívidas verificadas de código: middleware fail-closed (E9) e desalinhamento de stack §2.5 (E10: Next 15.3.3→16, React 19.1→19.2, shadcn/ui, Zustand, Vercel AI SDK v5, a11y CI).
...
- **Infra** (`platform-infra-engineer`): plataforma-base validada offline; 2 gaps de código: `policy-aml-kyc.hcl` faltante e item 4 do mandato (cosign/SBOM/Trivy/Falco) só 25% coberto (E8).
...
**Estado atual.** Registry código-completo e verde: rodei ao vivo `buf lint`/`buf format --diff --exit-code`/`buf breaking --against main`/`make verify` na raiz — todos passam sem diff contra `main` (5 pacotes: common, telemetry, decision, money, payments; envelope com os 4 campos críticos; Money int64+scale; Decision/Candidate/ExplorationPolicy para OPE; payments/v1 do K0 do ADR-0004 gerado em Go+TS). Isso fecha o mandato de TX-1 para Fases 0-1-2-3. Único achado verificado nesta sessão: `make proto-gen-check` falha hoje só nos artefatos TS por bump de versão do plugin remoto `protoc-gen-es` (v2.12.0→v2.12.1 no header `@generated by`), não por drift de schema — falso-positivo de staleness gate, registrado como próximo passo.
...
| `E3` | Decision log + propensão — fecha o loop de atribuição/OPE | ✅ concluída | `schema-contracts-steward` | `TX-1` · `§2.3` · `TX-4` · `DA-3` · `propensity-logging.md` | — |
...
| `E8` | Corrigir o falso-positivo de `proto-gen-check` (drift de versão de plugin remoto, não de schema) | → próxima | `schema-contracts-steward` | `TX-1` · `.github/workflows/buf.yml` · `Makefile (proto-gen-check)` | — |
...
**Subagente:** `schema-contracts-steward` · **Doc:** `TX-1` · `§2.3` · `TX-4` · `DA-3` · `propensity-logging.md` · **Gate:** buf lint + buf breaking verdes; contrato de propensão publicado e referenciado pelo README/proto README · **Depende de:** E1

##### E4 · Contratos em prosa cross-cutting de dinheiro (Money/Asset Registry/anti-float) — ✅ concluída

Garantir coerência semântica do tipo Money em todas as fronteiras (evento→ledger→BFF→UI) e a política de lint anti-float nas 4 linguagens, sem editar proto/ (fronteira de propriedade com money-ledger-guardian).
...
- contracts/lint/no-float.md: escopo restrito a money/ledger/billing/payments/asset_registry/migrations, scripts scripts/ci/no-float-{go,py,sql}.sh + ESLint override
- contracts/README.md como índice e fronteira explícita ("referenciam o schema, nunca editam proto/")

**Subagente:** `schema-contracts-steward` · **Doc:** `TX-2` · `DA-10` · `§2.6` · `§3` · **Gate:** no-float verde nas 4 linguagens (make verify); money-ledger-guardian valida a semântica contábil consumidora · **Depende de:** E1
...
##### E8 · Corrigir o falso-positivo de `proto-gen-check` (drift de versão de plugin remoto, não de schema) — → próxima

Eliminar o ruído do gate de staleness de CI: hoje ele falha nos artefatos TS por bump de versão do `protoc-gen-es` remoto (v2.12.0→v2.12.1, só o header `@generated by` muda), o que pode mascarar um drift real futuro se o time passar a ignorar falhas do gate por hábito.

- Investigar se `buf.build/bufbuild/es` (e `buf.build/protocolbuffers/go`) podem ser fixados por revisão/digest no `buf.gen.yaml` em vez de floating-latest, para que `proto-gen-check` só falhe em drift real de schema
- Se pin de revisão não for suportado pelo plugin remoto, avaliar rodar os plugins localmente (protoc-gen-es/protoc-gen-go vendorizados) como alternativa determinística
- Documentar no proto/README.md e no job de CI a distinção entre 'diff de schema' (deve falhar) e 'diff de versão de gerador' (não deveria falhar o merge)
- Regenerar e commitar gen/go+gen/ts uma vez com a versão atual do plugin para religar o gate ao verde
...
**→ Próximo plano deste addon.** Executar E8 primeiro (não bloqueado, puro trabalho de config/CI do meu domínio): investigar pin de revisão dos plugins remotos `buf.build/bufbuild/es`/`buf.build/protocolbuffers/go` no buf.gen.yaml (ou alternativa de plugin local vendorizado) para que `make proto-gen-check` pare de falhar por bump cosmético de versão do gerador (`protoc-gen-es` v2.12.0→v2.12.1, confirmado nesta sessão como o único diff — só o header `@generated by`), e religar o gate ao verde regenerando/commitando gen/go+gen/ts uma vez. Em paralelo, E9 (ativação em produção real) e E10/E11 (near-real-time/Flink; multi-touch) permanecem 'gated' aguardando, respectivamente: infra viva aplicando platform/ + cutover real (E9), e os dois gatilhos mensuráveis explícitos dos ADR-0001/ADR-0002 §B.7 (E10/E11) — nenhum dos três é código pendente do schema registry hoje; o trabalho de schema em si (E1-E7) está código-completo, provado ao vivo (`buf lint`/`buf breaking`/`make verify` verdes contra `main` nesta sessão) e sem diff pendente.
...
| `E3` | Wiring local + suíte golden de paridade completa (I5 + 16ª onda) | ✅ concluída | `decision-engine-engineer` | `ADR-0002 §D (I4, I5)` · `CA-2` · `CA-3` · `CA-4` · `CA-5` · `CA-6` · `stack §5 (risco 'reescrita Go divergir da semântica legada')` | — |
...
| `E5` | Hot-reload do GeoLite2 (.mmdb) sem restart — fecha CA-9 do lado do motor | ◻ pendente | `decision-engine-engineer` | `DA-9` · `CA-9` · `§4.10` | validação end-to-end depende de um job externo de download/rotação automática do .mmdb com chave de licença MaxMind real (platform-infra-engineer); o hot-reload em si é código-endereçável agora com um arquivo de teste local. |
| `E6` | Correção HOT-1/HOT-3 — RankResult por-request em vez de campo `last` compartilhado | → próxima | `decision-engine-engineer` | `TX-4` · `DA-3` · `ADR-0003 §G (J3, J4)` · `ADR-0004 §A` | — |
...
**Subagente:** `decision-engine-engineer` · **Doc:** `ADR-0002 §D (I4, I5)` · `CA-2` · `CA-3` · `CA-4` · `CA-5` · `CA-6` · `stack §5 (risco 'reescrita Go divergir da semântica legada')` · **Gate:** parity-golden-test-guardian aprova a suíte completa (5/5 golden CA-mapeados, harness shadow/dual-run presente); make go-test -race verde em todos os pacotes deste addon. · **Depende de:** —
...
##### E5 · Hot-reload do GeoLite2 (.mmdb) sem restart — fecha CA-9 do lado do motor — ◻ pendente

Substituir o carregamento único no boot (documentado como aceitável só para o MVP) por um mecanismo de hot-reload do arquivo .mmdb já atualizado em disco, sem derrubar o processo de decisão.

- internal/geo: adicionar reload via os.Signal (ex.: SIGHUP) ou verificação periódica de mtime do arquivo, trocando o *maxminddb.Reader atomicamente (RWMutex/atomic.Pointer) sem interromper requests em voo
- cobrir com teste que o Resolver antigo continua servindo durante a troca (zero downtime) e que um arquivo corrompido não derruba o processo (mantém o reader anterior)
- documentar no runbook o sinal/gatilho de reload esperado pelo job externo de auto-atualização
...
##### E6 · Correção HOT-1/HOT-3 — RankResult por-request em vez de campo `last` compartilhado — → próxima
...
**→ Próximo plano deste addon.** Ativação em produção (E7): destravar o cutover assim que platform/ estiver aplicada em cloud, a chave MaxMind real disponível e o dual-run contábil provar paridade com o Revive legado dentro da tolerância — sequenciado depois de fechar os dois itens código-endereçáveis que ainda restam (E5 hot-reload do GeoLite2, E6 HOT-1/HOT-3) para não abrir a porta de produção com uma correção de concorrência pendente. Em seguida, E8 liga os flags de ML (ranker/AB/shadow/deep) só sob tráfego real e uplift A/B provado — nunca por aspiração. Sucessor pós-Fase-3 (E9): reabertura do orçamento de 5–8 ms/25 ms só sob número de p99 medido e sustentado, com ADR sucessor explícito; nenhuma tecnologia pesada (Rust+Axum, Treelite in-process/CGO) entra sem esse gatilho.
...
**Estado atual.** Todos os incrementos de código do addon estão fechados na main: J0 (propensão instrumentada), J1 (re-ranker + sidecar fail-open), J2 (treino pCTR + calibração + MLflow), J3 (OPE + shadow), J4 (A/B + kill-switch + promoção gated), J6 (pacing proporcional + fraude/IVT supervisionada), K1 (deep scaffolding default-off) e K2 (fraude não-supervisionada) — todos com gates verdes (parity/security/privacy/money) reafirmados em ondas sucessivas de hardening. Dois itens de código genuinamente pendentes (não gated por infra): a correção HOT-1/HOT-3 (atribuição de RankResult por-request, hoje via campo `last` compartilhado) documentada como pré-condição bloqueante antes de ligar tráfego real, e o ONNX Runtime nativo no sidecar (hoje `StubInferencer`, score=0). K8 (promoção do deep) e a ativação real dos flags seguem gated por tráfego real/infra da Fase 1-2.
...
| `E10` | Correção HOT-1/HOT-3 — atribuição de RankResult por-request (pré-condição de código antes de tráfego real) | → próxima | `ml-optimization-engineer` | `README.md — 'Pendente da Fase 3' (HOT-1/HOT-3)` · `ADR-0003 §A (fail-open/DA-3)` · `CA-2` | — |
| `E11` | ONNX Runtime nativo no sidecar (substituir StubInferencer por OnnxInferencer) | ◻ pendente | `ml-optimization-engineer` | `ADR-0003 §B` · `README.md — Fase 2 pendências ('ONNX Runtime nativo no sidecar, hoje stub')` · `TX-4` | infra: CGO + libonnxruntime.so.1 indisponíveis neste ambiente de build; o wrapper Go é escrito e testado sob build tag, mas o binário linkado (CGO) só é validado onde a lib nativa existir |
...
##### E10 · Correção HOT-1/HOT-3 — atribuição de RankResult por-request (pré-condição de código antes de tráfego real) — → próxima
...
| `E12` | Higiene de layout — resolver os pacotes reservados vazios guardrails/ e rag/ | ◻ pendente | `copilot-llm-engineer` | `ADR-0003 §F (política de layout — não mandata subpastas dentro de services/copilot/, só o serviço como um todo)` | — |
...
**Subagente:** `copilot-llm-engineer` · **Doc:** `stack-tecnologico.md §2.4` · `ADR-0003 §C` · `ADR-0003 §G (linha J5)` · **Gate:** tech-lead-architect (design do grafo) + parity-golden-test-guardian (não altera hot path/faturável) — PASS; 126 pytest do copiloto verdes · **Depende de:** —
...
##### E10 · Ativação em produção — plugar credenciais e infra vivas atrás dos pontos de extensão já prontos — ◻ pendente
...
##### E12 · Higiene de layout — resolver os pacotes reservados vazios guardrails/ e rag/ — ◻ pendente
...
**Estado atual.** O console (web/console) e o BFF (bff/src) estão CÓDIGO-COMPLETOS para os incrementos I4 (Fase 1), J5 (Fase 2) e K7 (Fase 3 §H): CRUD com ACL server-side (CA-1), builder de segmentação com anti-contradição (CA-4), dashboards que nunca somam "ao vivo" com "consolidado ≤1h" (CA-6/ADR-0001), dinheiro tratado como string DECIMAL de ponta a ponta sem Number()/aritmética no cliente (TX-2), copiloto com streaming SSE + HITL obrigatório (ADR-0003 J5), e UI/BFF de pagamentos somente-leitura sem cripto no cliente (ADR-0004 K7). Todos os gates de segurança/privacidade/paridade citados no README (12ª–16ª onda) estão verdes para este componente; `make bff-ci`/`make web-ci` passam. A varredura de código desta sessão encontrou 2 desvios reais e verificados vs. o mandato §2.5, ainda não corrigidos: (1) o middleware de sessão tem modo dev-stub sem hard-fail explícito se `NODE_ENV=production` e `SESSION_SECRET` ausente; (2) versões pinadas abaixo do mandato (Next 15.3.3/React 19.1.0 vs. Next 16/React 19.2), ausência de shadcn/ui+Base UI/React Aria como design system, ausência de Zustand, copiloto usa parser SSE hand-rolled em vez do Vercel AI SDK v5 ratificado no ADR-0003, e nenhum teste automatizado de acessibilidade (axe/Playwright) em CI apesar do mandato WCAG 2.2 AA.
...
- bff-ci cobre 73 testes (config-adapter + payments-adapter + in-memory fixtures)
...
**Estado atual.** A plataforma-base está CÓDIGO-COMPLETA e verificada offline (`tofu validate`, `kubeconform`, `kyverno test`, `otelcol validate` — todos espelhados em `make platform-validate` e no CI `.github/workflows/platform.yml`, com SHA256 fixado nos downloads — M-1). Isso fecha a Fase 0 (plataforma-base) e os incrementos K0/K4/K6 do ADR-0004 (células PCI e AML/KYC). Duas ondas de hardening (11ª e 13ª, README) já corrigiram bugs de CI que mascaravam falha (pipefail ausente no tofu-validate; endpoints placeholder faltando no otel-validate). Nesta varredura encontrei 2 gaps de código genuínos e pequenos (não bloqueados por infra): falta o arquivo `platform/secrets/openbao/policy-aml-kyc.hcl` (só documentado em comentário) e o mandato item 4 (cosign+SBOM+Kyverno+Trivy+Falco) só tem 1 de 4 controles com código real — Kyverno admite/rejeita, mas não há workflow de build+SBOM+assinatura+scan de imagem nem Dockerfiles de produção por serviço (só `deploy/local/Dockerfile` de dev existe), e a chave pública cosign em `kyverno-baseline.yaml` é um placeholder (`REPLACE_WITH_COSIGN_PUBLIC_KEY`).
...
| `E8` | Fechar gaps de supply chain do mandato (cosign real + SBOM + Trivy + Falco) — construível agora, sem cloud | → próxima | `platform-infra-engineer` | `mandato item 4` · `§2.7` · `CA-9` | a construção dos Dockerfiles de produção por serviço é compartilhada com os donos de camada (decision-engine-engineer, data-platform-engineer, ml-optimization-engineer, payments-crypto-engineer, frontend-bff-engineer); platform-infra-engineer entrega o pipeline/template de CI, não a imagem de cada app |
...
- `platform/observability/otel-collector.yaml`: pipelines traces/logs/metrics com `transform/redact-pii` + `redaction/allowlist-{traces,logs}` (`allow_all_keys: false` fail-closed em ambas)
- `make platform-otel-validate`: `otelcol validate` via imagem contrib pinada (SHA/tag fixados) + verificação estrutural fail-closed (grep) da presença dos processadores TX-5

**Subagente:** `platform-infra-engineer` · **Doc:** `TX-5` · `DA-11` · **Gate:** privacy-compliance-auditor PASS — pipelines contêm redact-pii E allowlist, allow_all_keys false em ambas (checklist runbook §6) · **Depende de:** E1
...
##### E8 · Fechar gaps de supply chain do mandato (cosign real + SBOM + Trivy + Falco) — construível agora, sem cloud — → próxima

Completar o item 4 do mandato: hoje só Kyverno tem código real; faltam os outros 3 controles e um pequeno gap de policy.

- Escrever `platform/secrets/openbao/policy-aml-kyc.hcl` (path `aml-kyc/*` read-only) — hoje só existe como comentário no `openbao-auth.yaml`
- Desenhar workflow de build de imagem por serviço (Dockerfiles de produção para `services/decision`, `collector`, `ranker-sidecar`, `copilot`, `payments` — hoje só há `deploy/local/Dockerfile` de dev) com etapas: build → SBOM (syft/cyclonedx) → cosign sign (keyless OIDC ou chave gerenciada) → Trivy scan (fail em CRITICAL) → push `ghcr.io`
- Gerar par de chaves cosign real (ou adotar keyless) e substituir `REPLACE_WITH_COSIGN_PUBLIC_KEY` em `kyverno-baseline.yaml`
- Autorar ruleset Falco customizado como manifest declarativo (regras mínimas: exec em container privilegiado, escrita fora de paths esperados, syscalls suspeitos nas células PCI/AML) — o deploy real do daemonset é pós-cluster (E9), mas o ruleset é código hoje
...
| `E7` | Gate de CI no-float — piso, não teto (4 linguagens + detecção de furos) | ✅ concluída | `money-ledger-guardian` | `TX-2` · `.github/workflows/no-float.yml` | — |
...
Estabelecer Money(asset_code, integer_amount, scale) como único tipo monetário atravessando fio/ledger/BFF/UI, com float banido em CI nas 4 linguagens.
...
- Lint contracts/lint/no-float.md + 4 scripts scripts/ci/no-float-{go,py,sql,data-sql}.sh + workflow .github/workflows/no-float.yml
- Helpers Go internal/money (Compare/ECPMMinorUnits/IsZero/GreaterThan) com panic em cross-currency

**Subagente:** `money-ledger-guardian` · **Doc:** `TX-2` · `DA-10` · `contracts/money/money-type.md` · `contracts/lint/no-float.md` · **Gate:** money-ledger-guardian — no-float verde nas 4 linguagens (CI); zero float em campo monetário · **Depende de:** —
...
##### E7 · Gate de CI no-float — piso, não teto (4 linguagens + detecção de furos) — ✅ concluída
...
- Aplicar db/asset_registry/migrations/0001, db/ledger/migrations/0001+0002+0003 na ordem do runbook (§2.1/§2.3)
...
- Re-rodar make ml-deep-test (K1, 22 testes) a cada onda — invariante DEEP_ENABLED=false até uplift A/B provado (K8).
...
- transform/redact-pii DELETA client.address/http.client_ip/net.sock.peer.addr/net.peer.ip/url.full/enduser.id/user.id/user.email em traces E logs (platform/observability/otel-collector.yaml:31-62).
- Allowlist fail-closed (allow_all_keys:false) em traces e logs: qualquer chave nova/inesperada e bloqueada por padrao (otel-collector.yaml:67-132).
- blocked_values mascaram IPv4 e e-mail como defesa em profundidade nos dois pipelines (otel-collector.yaml:84-86,129-131); capping.key hasheada com SHA256 se aparecer (otel-collector.yaml:47,62).
- COBERTURA DE TODOS OS EXPORTERS: pipelines traces e logs incluem redact-pii + allowlist antes de batch/export (otel-collector.yaml:150-166). ATENCAO: o pipeline de METRICS (linhas 156-159) NAO passa por redacao - exigir prova de que labels de metrica jamais carregam PII (so dimensoes estruturais tenant_id/zone_id/etc.).
- CI impoe a redacao: platform-otel-validate roda otelcol validate + grep fail-closed (make/platform.mk:197-206), bloqueando allow_all_keys:true e ausencia de redact-pii/allowlist-*.
...
- **`make verify` = buf (TX-1) + no-float (TX-2)**, offline e determinístico:
  ...
  - `no-float` nas 4 linguagens (Go/Py/SQL/data-SQL) no escopo `money/ledger/billing/payments/chainconnector` + `ml/fraud|pacing` (`scripts/ci/no-float-*.sh`, `.github/workflows/no-float.yml`). **`float` proibido para dinheiro (TX-2), é piso não teto.**
- `proto-gen-check` é job separado (depende de rede/plugins remotos, propositalmente fora de `verify` hermético) — hoje com falso-positivo a corrigir (schema E8).
...
- **`make platform-validate`** = `tofu validate` + `kubeconform` + `kyverno test` + `otelcol validate` (TX-5), com SHA256 fixado nos binários de CI (`.github/workflows/platform.yml`). Espelha `make verify` para a plataforma; hermético.
...
##### G0 — Fechar as pré-condições de CÓDIGO (sem infra, faz-se agora)
Estas são as únicas pendências de teclado; devem entrar **antes** de abrir a porta de produção.
- **decision-engine E6 + ml E10 — HOT-1/HOT-3 (RankResult por-request).** Bloqueante: `parity` E14 recusa avaliar a promoção K8 enquanto o campo `last` compartilhado puder enviesar o OPE sob concorrência. Donos: `decision-engine-engineer` + `ml-optimization-engineer`; gate `parity-golden-test-guardian`. Ref: `ADR-0003 §G (J3/J4)`, `TX-4`, `DA-3`, README (HOT-1/HOT-3).
- **decision-engine E5 — hot-reload do GeoLite2 (.mmdb)** sem restart (RWMutex/atomic.Pointer). Dono: `decision-engine-engineer`. Ref: `DA-9`, `CA-9`, `§4.10`.
- **ml E11 — ONNX Runtime nativo** (OnnxInferencer sob build tag, preservando build hermético `ADR-0002 §C`). Dono: `ml-optimization-engineer`; gate `tech-lead-architect` + `parity`. Ref: `ADR-0003 §B`.
- **schema E8 — corrigir falso-positivo do `proto-gen-check`** (pin de revisão dos plugins remotos ou plugin local vendorizado). Dono: `schema-contracts-steward`. Ref: `TX-1`, `.github/workflows/buf.yml`.
- **platform-infra E8 — fechar item 4 do mandato:** `platform/secrets/openbao/policy-aml-kyc.hcl` faltante; pipeline cosign real + SBOM (syft) + Trivy + ruleset Falco + Dockerfiles de produção por serviço; substituir `REPLACE_WITH_COSIGN_PUBLIC_KEY`. Dono: `platform-infra-engineer` (coadjuvado pelos donos de camada nos Dockerfiles). Ref: mandato item 4, `§2.7`, `CA-9`.
- **frontend E9 + E10 — fail-closed do middleware** em produção sem `SESSION_SECRET` **e** alinhamento de stack §2.5 (Next 15.3.3→16, React 19.1→19.2, shadcn/ui sobre Base UI/React Aria, Zustand, Vercel AI SDK v5, axe+Playwright em CI). Dono: `frontend-bff-engineer`; gate `security-reviewer` + `tech-lead-architect`. Ref: `§2.5`, `WCAG 2.2 AA`, `TX-3`.
- **copiloto E12 — higiene de layout** (resolver pacotes vazios `guardrails/`/`rag/`). Dono: `copilot-llm-engineer`; gate `tech-lead-architect` (baixo risco).
...
- **money-ledger E9:** aplicar migrations `0001+0002+0003` do ledger + asset_registry na ordem do runbook; **USDC scale=6 antes do 1º depósito**; Reconciler→Iceberg real. Ref: `runbook §2.3/§6`.
...
- Incremento **I5** citado como 'ADR-0002 §D (I4, I5)' (plano decision-engine, stage E3/completed_increments) e 'ADR-0002 (I5)' (plano frontend, E11): NÃO existe no ADR-0002 §D, que sequencia explicitamente apenas I0–I4 ('A Fase 1 é construída em 5 incrementos', 'I4 fecha o ciclo'). I5 (loader Postgres→snapshot / wiring local) está definido só em README.md (linha 74/85), sem emenda ao ADR. Ação: corrigir a ref para 'README.md (I5)' OU emendar ADR-0002 §D formalizando I5 como 6º incremento — a atual mistura a fonte normativa (ADR) com a fonte de-facto (README).

- docs/documentacao-tecnica.md §5 (CA-1…CA-9): TODOS os checkboxes '- [ ]' seguem desmarcados apesar dos goldens verdes e das evidências executáveis já produzidas (tests/parity/*, db/*/tests, runbook §6). Incoerência de canonicidade sinalizada pelo próprio plano de paridade (E10): o README virou fonte-de-verdade de fato e o §5 normativo ficou estagnado. Requer adjudicação do tech-lead-architect sobre qual documento é canônico — hoje o §5 subrepresenta o estado real.

- make/parity.mk: o comentário do alvo lista 'parity-golden (CA-2, CA-4, CA-5, CA-6…)' OMITINDO CA-3, embora o golden CA-3 (tests/parity/ca3_creatives_golden_test.go, fechado na 16ª onda) já exista e rode dentro do mesmo 'go test ./tests/parity/...'. Drift cosmético de comentário (a execução roda os 5 goldens) — atualizar o texto do alvo para citar CA-3 e evitar mascarar futuramente um golden faltante.

- Ref bare '§2.3' no plano do schema-contracts-steward (stage E3 — decision log/propensão/OPE) sem qualificador de documento: em docs/documentacao-tecnica.md NÃO há §2.3 (só §2.1 'Dentro do escopo' e §2.2 'Fora do escopo'); a §2.3 que trata de IA/DL/OPE vive em docs/stack-tecnologico.md. Qualificar como 'stack §2.3' para remover a ambiguidade (o addon cita ambos os docs no doc_layer).

- Nota de coerência código↔doc (não é ref errada, mas desvio verificado): o plano do frontend-bff-engineer reconhece que web/console está pinado em Next 15.3.3/React 19.1.0 enquanto stack §2.5 mandata Next 16/React 19.2 + shadcn/ui + Zustand + Vercel AI SDK v5 + axe/Playwright. A ref (§2.5) está correta; o CÓDIGO é que diverge do mandato. Endereçado no G0/E10 do próximo plano — registrar para não ser lido como conformidade.
