...
//  4. Finalidade (N confirmacoes) -> ledger.FinalizeDeposit (pending->posted).
...
//
// Move a entry de deposito de pending -> posted no ledger.
...
//
// A submissao real (chamada HTTP ao Safe Transaction Service) e marcada
// como STUB: depende de chave viva (SAFE_TX_SERVICE_URL + signers) que
// pende de infra (nao implementada ate K6/infra viva).
//
// Idempotencia: payout:{payment_event_id} no ledger (K3).
//
// STUB: a submissao HTTP ao Safe Transaction Service e implementada como
// comentario/placeholder ate infra viva estar disponivel.
func (h *SafeWebhookHandler) SubmitPayout(ctx context.Context, req chainconnector.PayoutRequest, connector chainconnector.ChainConnector) error {
	...
	// Constroi a tx nao-assinada via ChainConnector
