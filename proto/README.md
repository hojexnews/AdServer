...
## Staleness gate (`proto-gen-check`) — versao de plugin FIXADA

O `make proto-gen-check` (CI: `.github/workflows/buf.yml`) regenera `gen/` num
tmpdir e compara com o versionado. Ele existe para pegar **drift de schema**
(um `.proto` mudou mas `gen/` nao foi regenerado) — isso DEVE falhar o merge.

Os plugins remotos em `buf.gen.yaml` sao **fixados por versao**
(`buf.build/protocolbuffers/go:v1.36.11`, `buf.build/bufbuild/es:v2.12.0`)
justamente para que o gate **nao** falhe por **diff de gerador** — um bump
cosmetico de um plugin `latest` flutuante que muda apenas o header
`@generated by` (ex.: `protoc-gen-es v2.12.0 -> v2.12.1`), sem tocar o schema.
Um diff assim NAO deve reprovar o merge, e o pin o elimina.

Para subir de versao do gerador: atualize o pin em `buf.gen.yaml`
deliberadamente, rode `make proto-gen` e commite `gen/` no mesmo PR.
